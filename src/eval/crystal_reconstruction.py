...
class CrystalReconstructionEvaluator:
    ...
    def get_metrics(self, save: bool = False, save_dir: str = "") -> Dict[str, Any]:
        ...
        # For CSP, atom types are given — only check struct_valid (geometry).
        # Excluding comp_valid (SMACT) which incorrectly gates match_rate for
        # exotic/intermetallic compositions that fail charge balance tests.
        validity = [
            c1.struct_valid and c2.struct_valid
            for c1, c2 in zip(self.pred_crys_list, self.gt_crys_list)
        ]
    ...
    def get_wandb_table(self, current_epoch: int = 0, save_dir: str = "") -> wandb.Table:
        ...
        for idx in range(len(self.pred_crys_list)):
            ...
            rmsd_val = self.rms_dists[idx].item()  # tensor → plain Python float
            # inf displayed as 0 in WandB — use None instead so cell shows as empty
            rmsd = None if not (rmsd_val < float("inf")) else rmsd_val
            ...
            pred_table.add_data(
                ...
                rmsd,
                match,
                sorted(self.pred_crys_list[idx].atom_types) == sorted(self.gt_crys_list[idx].atom_types),
