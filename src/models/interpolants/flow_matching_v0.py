...
class FlowMatchingInterpolant:
    ...
    # def _centered_gaussian(self, batch_size, num_tokens, emb_dim=3, atom_types=None): ### RZD
    #     noise = torch.randn(batch_size, num_tokens, emb_dim, device=self.device)  ### (B=256 N=20 d=8)
    #     # print(noise.shape)
    #     # print(batch_size, emb_dim)
    #     return noise - torch.mean(noise, dim=-2, keepdims=True)

    def _centered_gaussian(self, batch_size, num_tokens, emb_dim=3, atom_types=None):  ### RZD
        """
        Args:
            batch_size: (int) Batch size (ignored if atom_types is provided).
            num_tokens: (int) Max tokens (ignored if atom_types is provided).
            emb_dim: (int) Embedding dimension.
            atom_types: (torch.Tensor) Dense tensor of shape [Batch, Max_Nodes].
        """
        # 1. If atom_types is provided, override batch_size and num_tokens
        # if atom_types is not None:
        #     batch_size, num_tokens = atom_types.shape

        # 2. Generate the random noise container [Batch, Max_Tokens, Dim]
        noise = torch.randn(batch_size, num_tokens, emb_dim, device=self.device)

        if atom_types is not None:
            # 3. Inject atom_types into the first component (index 0)
            # Ensure types are cast to float to match the noise tensor
            noise[:, :, 0] = atom_types.to(device=self.device, dtype=noise.dtype)

            # 4. Zero-Center ONLY the spatial coordinates (indices 1 to end)
            # We don't want to center/shift the atom types in index 0.
            spatial_mean = torch.mean(noise[:, :, 1:], dim=-2, keepdims=True)
            noise[:, :, 1:] = noise[:, :, 1:] - spatial_mean

            # Optional: Mask out padding in the noise if atom_types uses 0 as padding
            # mask = (atom_types != 0).unsqueeze(-1)
            # noise = noise * mask

            return noise
        else:
            # Standard behavior for non-CSP tasks
            return noise - torch.mean(noise, dim=-2, keepdims=True)

    ### during the corrupt process, keep the first atom_type unchanged!!!
    ### in the VAE decoder part, fix the first component to be atom_type and just read it to reconstruct cif

    def _corrupt_x(self, x_1, t, token_mask, diffuse_mask, atom_types=None):  ### RZD
        # print('***************debug only********')
        # print(x_1, x_1.shape)
        x_0 = self._centered_gaussian(*x_1.shape, atom_types)  ### RZD
    ...
    def corrupt_batch(self, batch):
        ...
        #####
        atom_types = batch.get("atom_types", None)  ### RZD
        ...
        # Apply corruptions
        if self.corrupt:
            x_t = self._corrupt_x(x_1, t, token_mask, diffuse_mask, atom_types)  ### RZD
        else:
            x_t = x_1
    ...
    def sample(
        ...
        if x_0 is None:
            x_0 = self._centered_gaussian(batch_size, num_tokens, emb_dim, atom_types)  ### RZD
