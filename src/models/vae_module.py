...
class VariationalAutoencoderLitModule(LightningModule):
    ...
    def __init__(
        ...
        self.quant_conv = torch.nn.Linear(
            self.encoder.d_model, 2 * self.latent_dim_cont, bias=False
        )
    ...
    ### modified by RZD
    def atom_types_to_ztype(self, atom_types: torch.Tensor) -> torch.Tensor:
        # atom_types: (total_nodes,) int64 with 0 used as padding/unknown in this repo
        # map to float; simplest: keep as float id
        return atom_types.float().unsqueeze(-1)

    def ztype_to_atom_types(self, z_type: torch.Tensor) -> torch.Tensor:
    ...
    def encode(self, batch):
        ...
        # deterministic type channel derived from input
        atom_types_src = getattr(batch, "atom_types_clean", batch.atom_types)
        encoded_batch["z_type"] = self.atom_types_to_ztype(atom_types_src)
    ...
    ### modified by RZD
    def decode(self, encoded_batch):
        # encoded_batch["x"] is (total_nodes, d+1)
        z_type = encoded_batch["x"][..., :1]
        z_cont = encoded_batch["x"][..., 1:]

        encoded_batch["x"] = self.post_quant_conv(z_cont)  # back to decoder.d_model
        out = self.decoder(encoded_batch)

        # Replace predicted type logits with deterministic logits from z_type
        # out["atom_types"] currently expected to be logits (N, num_types) used in CE loss
        atom_type_ids = self.ztype_to_atom_types(z_type)  # (N,)
        num_classes = out["atom_types"].shape[-1]

        # Create very confident logits so argmax = atom_type_ids
        forced_logits = torch.full_like(out["atom_types"], -1e9)
    ...
    def forward(self, batch: Data, sample_posterior: bool = True):
        ...
        if DEBUG_VAE_LATENT:
            print(
                ...
                batch.atom_types[:10].detach().cpu().tolist(),
            )
        ...
        z_type = encoded_batch["z_type"]  # (total_nodes, 1)
        ...
        if DEBUG_VAE_LATENT:
            ...
            print(
                "[DEBUG] Latent first channel (first 10):",
                encoded_batch["x"][:10, 0].detach().cpu().tolist(),
            )
    ...
    def training_step(self, batch: Data, batch_idx: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            if self.hparams.augmentations.noise > 0.0:
                ...
                ###modified by RZD
                # save original atom types (keep for undo step)
                atom_types_ = batch.atom_types.clone()
                # expose clean types to encoder, in case other code later mutates batch.atom_types
                batch.atom_types_clean = atom_types_
                # NOTE: atom-type corruption disabled for this VAE variant (type-channel latent).
                # We still apply position corruption below.
