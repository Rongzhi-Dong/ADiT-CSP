...
@torch.no_grad()
def ema_update(ema_model_tuple, current_model_tuple, decay):
    # Filter to float tensors only — integer params (e.g. embedding indices)
    # cannot be EMA-updated with a float decay
    float_ema = [p for p in ema_model_tuple if p.is_floating_point()]
    float_cur = [p for p in current_model_tuple if p.is_floating_point()]
    torch._foreach_mul_(float_ema, decay)
    torch._foreach_add_(float_ema, float_cur, alpha=(1.0 - decay))
